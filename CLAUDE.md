...
## 予約フォーム(GAS + Googleカレンダー連携)— 稼働中
- Access セクションの `#reservationForm`(氏名・電話・メール・メニュー・希望日時・備考)から、`index.html` 内の `RESERVATION_ENDPOINT` に設定済みのGAS Webアプリ(`gas/Code.gs`をユーザーがscript.google.comにデプロイしたもの)へPOSTする構成が本番稼働中。
- GAS側で営業時間/定休日バリデーション・ダブルブッキング確認・カレンダー登録(`CalendarApp.getDefaultCalendar()`)・お客様&オーナーへの確認メール送信までを実施。
- 2026-08-19、実データでのテスト予約2件(本日13:00〜/8月20日11:00〜)で、カレンダー登録・メール送信・レスポンスの3点をユーザー自身が実カレンダー・実メールで確認済み。テスト予約はユーザーが手動削除する想定(削除確認は未追跡)。
...
- 予約フォーム(GAS + Googleカレンダー連携)本番稼働開始、実テスト予約で動作確認済み(2026-08-19)
