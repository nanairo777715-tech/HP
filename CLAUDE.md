...
- モチーフ: 「爪先(アーチ)」の形をスウォッチ・カード・ボタンに一貫使用。ただしHeroの「This Season's Palette」6色スウォッチのみ、ユーザー指定により丸型・小サイズ(1列表示、`clamp(34px,8vw,52px)`)に変更済み(2026-09-02)
...
- Heroの「This Season's Palette」6色スウォッチを、爪先アーチ型から丸型・小サイズ(1列表示)に変更(2026-09-02)。ユーザーがポートフォリオサイトの編集中に「大きすぎる」と気づいたため
- 「This Season's Palette」を8色に拡張し、ブルベ向け4色・イエベ向け4色の2段組に変更(2026-09-02)。新色「スモークブルー」(ブルベ側)と「テラコッタ」(秋らしさ、イエベ側)を追加し、既存6色を両グループに振り分け。ユーザー確認済み(「違和感なし」)。**見出しに「(寒色系)/(暖色系)」を付けていたが、各グループ内に実際は寒暖混ざった色があるため不正確として削除**(ラベルは「ブルベ向け」「イエベ向け」のみ)
